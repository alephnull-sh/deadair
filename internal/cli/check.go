...
func runCheck(args []string, stdout, stderr io.Writer) int {
	...
	for _, inst := range insts {
		...
		if err != nil {
			...
			if withRetention > 0 {
				fmt.Fprintf(stdout, "  %s retention visible (%d of %d sources)\n", mark(stdout, true), withRetention, len(sources))
			} else {
				fmt.Fprintf(stdout, "  - retention not visible (optional; add read_ilm for truncated-lookback checks)\n")
			}
			if len(sources) > 0 {
