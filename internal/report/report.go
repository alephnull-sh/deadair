...
const (
	ReasonTruncatedLookback = "truncated-lookback" // queries further back than data is kept
	ReasonMissingFields     = "missing-fields"     // filters on fields no matched source provides
	ReasonLagBlindWindow    = "lag-blind-window"   // ingest lag exceeds the query-window margin
)
...
func impairment(rule backend.Rule, matched []string, srcByName map[string]backend.Source, fields map[string]map[string]bool) (ImpairedDetection, bool) {
	...
	// truncated-lookback: only when every matched source has a KNOWN
	// retention shorter than the rule's lookback.
	if rule.Lookback > 0 {
		short, minRet := true, time.Duration(0)
		for _, name := range matched {
			ret := srcByName[name].Retention
			if ret == 0 || ret >= rule.Lookback {
				short = false
				break
			}
			if minRet == 0 || ret < minRet {
				minRet = ret
			}
		}
		if short {
			imp.Reasons = append(imp.Reasons, ReasonTruncatedLookback)
			imp.LookbackSeconds = rule.Lookback.Seconds()
			imp.RetentionSeconds = minRet.Seconds()
		}
	}
	...
	// lag-blind-window: measured ingest lag exceeds lookback minus interval,
	// so events can land after their window was already queried.
	if rule.Lookback > 0 && rule.Interval > 0 && rule.Lookback > rule.Interval {
		margin := rule.Lookback - rule.Interval
		var maxLag time.Duration
		for _, name := range matched {
			if lag := srcByName[name].IngestLag; lag != nil && *lag > maxLag {
				maxLag = *lag
			}
		}
		if maxLag > margin {
