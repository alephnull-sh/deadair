...
func backendMetadata(name, observedVersion string) BackendMetadata {
	...
	case "sentinel":
		...
		details[CapabilitySourceLineage] = "ARM-only structural lineage from Log Analytics summary-rule inputs through the transform to its Analytics output table remains separate from bounded schedule-aware latest-completed-run LASummaryLogs evidence for relevant summary rules; invalid runtime configuration and overdue successes stay incomplete; both sections are informational, and native runtime failures do not create findings or gates"
