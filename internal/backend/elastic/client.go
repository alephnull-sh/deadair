...
func (c *Client) Sources(ctx context.Context) ([]backend.Source, error) {
	...
	// Doc counts per data stream, summed from backing indices; backing names
	// kept for retention lookup.
	streamDocs := make(map[string]int64)
	streamBacking := make(map[string][]string)
	...
	for _, ci := range cats {
		if m := backingRe.FindStringSubmatch(ci.Index); m != nil {
			...
			streamBacking[m[1]] = append(streamBacking[m[1]], ci.Index)
			continue
		}
	...
	c.fillRetention(ctx, sources, streamBacking)
	if c.MeasureLag {
...
// fillRetention resolves each source's configured deletion horizon from
// data-stream lifecycles and ILM delete phases. Metadata reads only; any
// failure leaves retention unknown and disables lookback checks for that
// source.
func (c *Client) fillRetention(ctx context.Context, sources []backend.Source, streamBacking map[string][]string) {
	// Data-stream lifecycle (DLM) retention.
	var ds struct {
		DataStreams []struct {
			Name      string `json:"name"`
			Lifecycle struct {
	...
	dlm := map[string]time.Duration{}
	if err := c.do(ctx, http.MethodGet, c.ESURL, "/_data_stream", nil, &ds); err == nil {
		for _, d := range ds.DataStreams {
			if r := backend.ParseInterval(d.Lifecycle.DataRetention); r > 0 {
				dlm[d.Name] = r
			}
		}
	}

	// ILM: index -> policy, then each policy's delete min_age.
	var explain struct {
		Indices map[string]struct {
			Policy string `json:"policy"`
		} `json:"indices"`
	}
	indexPolicy := map[string]string{}
	if err := c.do(ctx, http.MethodGet, c.ESURL, "/*/_ilm/explain?only_managed=true", nil, &explain); err == nil {
		for name, e := range explain.Indices {
			indexPolicy[name] = e.Policy
		}
	}
	policyRetention := map[string]time.Duration{}
	retentionForPolicy := func(name string) time.Duration {
		if name == "" {
			return 0
		}
		if r, ok := policyRetention[name]; ok {
			return r
		}
		...
		var r time.Duration
		if err := c.do(ctx, http.MethodGet, c.ESURL, "/_ilm/policy/"+url.PathEscape(name), nil, &out); err == nil {
			if p, ok := out[name]; ok {
				r = backend.ParseInterval(p.Policy.Phases.Delete.MinAge)
			}
		}
		policyRetention[name] = r
		return r
	}

	for i := range sources {
		s := &sources[i]
		if r, ok := dlm[s.Name]; ok {
			s.Retention = r
			continue
		}
		// Plain index: its own policy. Data stream: the shortest retention
		// among its backing indices' policies.
		indices := streamBacking[s.Name]
		if len(indices) == 0 {
			indices = []string{s.Name}
		}
		var min time.Duration
		for _, idx := range indices {
			if r := retentionForPolicy(indexPolicy[idx]); r > 0 && (min == 0 || r < min) {
				min = r
			}
		}
		s.Retention = min
