...
func (c *Client) preparePredicateFreshness(ctx context.Context, catalog map[string]tableInfo, request backend.RulePredicateFreshnessRequest) (backend.RulePredicateFreshnessEvidence, string, sentinelSourceTarget) {
	...
	query := fmt.Sprintf("%s | where TimeGenerated >= ago(%dh) and TimeGenerated <= now() + %dm | where %s | summarize LastEvent=max(TimeGenerated)",
		target.queryReference, int(freshnessWindow/time.Hour), int(backend.FreshnessFutureSkew/time.Minute), selector.Expression)
	if request.Basis == backend.FreshnessIngestionTime {
		query = fmt.Sprintf("%s | extend IngestionTime=ingestion_time() | where IngestionTime >= ago(%dh) and IngestionTime <= now() + %dm | where %s | summarize LastEvent=max(IngestionTime)",
			target.queryReference, int(freshnessWindow/time.Hour), int(backend.FreshnessFutureSkew/time.Minute), selector.Expression)
	}
