...
func TestSourcesHandlesFutureTimestampEvidence(t *testing.T) {
	...
	mux.HandleFunc("/_cat/indices", func(w http.ResponseWriter, _ *http.Request) {
		fmt.Fprint(w, `[
			{"index":".ds-logs-future-000001","docs.count":"2","store.size":"10"},
			{"index":".ds-logs-only-future-000001","docs.count":"1","store.size":"10"},
			{"index":".ds-logs-future-error-000001","docs.count":"1","store.size":"10"},
			{"index":".ds-logs-future-unmapped-000001","docs.count":"1","store.size":"10"},
	...
	sources, err := (&Client{URL: srv.URL}).Sources(context.Background())
	if err != nil {
		t.Fatal(err)
	}
