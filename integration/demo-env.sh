...
# lag-blind-window: a stream whose events arrive ~45m after their event time.
# Kept off the logs-* prefix on purpose so broad prebuilt rules (threat-intel,
# etc.) don't match it and inherit the synthetic lag — only the demo rule below
# reads it, so the finding stays attributable and clean.
body=""; for i in $(seq 1 30); do body+="{\"create\":{}}\n{\"@timestamp\":\"$NOW\",\"event\":{\"ingested\":\"$LAGTS\"},\"message\":\"b$i\"}\n"; done
curl -s -u $AUTH -X PUT "$ES/batchy-events" -H "Content-Type: application/json" -d '{"mappings":{"properties":{"@timestamp":{"type":"date"},"event":{"properties":{"ingested":{"type":"date"}}}}}}' -o /dev/null
printf "$body" | curl -s -u $AUTH -X POST "$ES/batchy-events/_bulk" -H "Content-Type: application/x-ndjson" --data-binary @- -o /dev/null; echo "seeded batchy-events"
...
mkrule '{"rule_id":"demo-lag","name":"Batch-fed source watcher","description":"d","risk_score":47,"severity":"medium","type":"query","query":"*:*","index":["batchy-events*"],"from":"now-6m","interval":"5m","enabled":true}'
curl -s -u $AUTH -X POST "$ES/_refresh" -o /dev/null; echo refreshed

curl -s -u $AUTH -X POST "$ES/_security/api_key" -H "Content-Type: application/json" -d '{"name":"deadair-demo2","role_descriptors":{"deadair_monitor":{"cluster":["monitor"],"indices":[{"names":["*"],"privileges":["monitor","view_index_metadata","read"]}],"applications":[{"application":"kibana-.kibana","privileges":["feature_siem.read","feature_siemV2.read"],"resources":["space:default"]}]}}}' | python3 -c "import json,sys; print(json.load(sys.stdin)['encoded'])" > "$(dirname "$0")/demo-key"
echo "key ready"
