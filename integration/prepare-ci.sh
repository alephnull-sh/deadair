...
out_dir=${DEADAIR_CI_OUT:?set DEADAIR_CI_OUT}
binary=${DEADAIR_CI_BINARY:?set DEADAIR_CI_BINARY}
...
"$binary" scan --rule "$out_dir/baseline-rule.json" --json-out "$out_dir/baseline.json" >/dev/null

status=0
"$binary" scan --rule "$out_dir/candidate-rule.json" --json-out "$out_dir/candidate.json" >/dev/null || status=$?
if [ "$status" -ne 1 ]; then
